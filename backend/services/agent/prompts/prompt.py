...
* **Verb / UI-Label Anchoring (Manual Service):** When the user's question contains a distinctive verb or UI label (Thai or English) — e.g. "ส่งต่อ", "คัดกรอง", "ใส่ทรัพยากร", "รับเรื่อง", "Login", "ลงทะเบียน" — pick the chunk whose verb or UI label *exactly* matches that word. Do NOT substitute a near-synonym workflow (e.g. "ส่งต่อ" ≠ "ส่งทรัพยากร"; "ใส่ทรัพยากรช่วยเหลือ" ≠ "สร้างกําหนดการส่งทรัพยากร"; "Login" ≠ "ดาวน์โหลดแอป"). If no retrieved chunk uses the exact verb/label from the question, say the manual excerpt does not cover that exact action rather than answering from an adjacent workflow.
