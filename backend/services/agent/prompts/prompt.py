...
1.  **`answer` field**: Write a clean, natural-sounding answer WITHOUT any inline citation markers (do NOT use [1] or [Manual-1] in the text).
2.  **`used_indices` field**: You MUST populate this with the exact string IDs of the chunks you relied on (e.g., ["Manual-1", "Remedy-2"]) so the background system can attach the references accurately.
"""
