...
const TICKET_CODE_PATTERN = /\b[A-Z]{2,5}-\d{4}-\d{4}\b/g

function getStandaloneTicketCodes(text: string): string[] {
  const codes = text.match(TICKET_CODE_PATTERN) ?? []
  if (codes.length < 2) return []

  const remainingText = text.replace(TICKET_CODE_PATTERN, '').trim()
  const onlySeparatorsRemain = /^[\s,;:|/\\\-–—()[\]{}]*$/.test(remainingText)

  return onlySeparatorsRemain ? codes : []
}

function TicketCodeGroup({ codes }: { codes: string[] }) {
  return (
    <div className="space-y-2">
      <div className="text-xs font-medium text-gray-500">พบ {codes.length} รายการ</div>
      <div className="flex flex-wrap gap-2">
        {codes.map((code, index) => (
          <span
            key={`${code}-${index}`}
            className="rounded-md border border-red-100 bg-red-50 px-2.5 py-1 text-sm font-medium text-[#B71C1C]"
          >
            {code}
          </span>
        ))}
...
export function ChatMessage({ message }: ChatMessageProps) {
  ...
  const standaloneTicketCodes = isBot ? getStandaloneTicketCodes(message.text) : []
        ...
        >
          {isBot ? (
            <div className="break-words text-sm leading-relaxed">
              {standaloneTicketCodes.length > 0 ? (
                <TicketCodeGroup codes={standaloneTicketCodes} />
              ) : (
                <ReactMarkdown
                  ...
                  components={{
                    ...
                    ul: ({ children }) => (
                      <ul className="list-disc pl-5 my-1.5 last:mb-0 space-y-0.5">{children}</ul>
                    ),
                    ol: ({ children }) => (
                      <ol className="list-decimal pl-5 my-1.5 last:mb-0 space-y-0.5">{children}</ol>
                    ),
